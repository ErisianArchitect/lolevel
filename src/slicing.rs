...
pub const fn subslice<T>(slice: &[T], range: Range<usize>) -> &[T] {
    ...
    unsafe {
        std::slice::from_raw_parts(slice.as_ptr().add(range.start), len)
    }
}
...
pub const fn subslice_mut<T>(slice: &mut [T], range: Range<usize>) -> &mut [T] {
    ...
    let len = range.end - range.start;
    unsafe {
        std::slice::from_raw_parts_mut(slice.as_mut_ptr().add(range.start), len)
    }
}
...
pub const fn substr(s: &str, range: Range<usize>) -> &str {
    assert!(s.is_char_boundary(range.start) && s.is_char_boundary(range.end), "Not utf-8 boundary.");
    unsafe {
        transmute(subslice(s.as_bytes(), range))
    }
}
